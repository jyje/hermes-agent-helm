---
title: Team member
description: leader 팀에 참여하는 개별 member release 구성
---

<div class="example-meta">
  <div><strong>필수 Secret</strong>NVIDIA_API_KEY, DISCORD_BOT_TOKEN</div>
  <div><strong>오버레이</strong>values-team-member.yaml</div>
</div>

## 언제 사용하나요?

고유한 bot token, fullnameOverride, TEAM_MEMBER_NAME이 필요합니다.

## 설치

```bash
helm upgrade --install hermes-agent ./charts/hermes-agent \
  --namespace hermes-agent --create-namespace \
  -f charts/hermes-agent/values-team-member.yaml \
  --set-string env.NVIDIA_API_KEY='<real-value>' --wait
```

둘 이상의 자격 증명이 필요한 예제에서는 모든 값을 `--set-string`으로 전달하거나 `extraEnvFrom`으로 기존 Secret을 참조하세요.

## 배포 전 조정

member마다 이 파일로 별도 Helm release를 만들고 identity 값을 다르게 설정합니다.

[원본 YAML 열기](https://github.com/jyje/hermes-agent-helm/blob/main/charts/hermes-agent/values-team-member.yaml)

## 전체 오버레이

```yaml title="charts/hermes-agent/values-team-member.yaml"
--8<-- "charts/hermes-agent/values-team-member.yaml"
```