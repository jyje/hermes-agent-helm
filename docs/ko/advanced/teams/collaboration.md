...
[English](../../../advanced/teams/collaboration.md) · [한국어](collaboration.md)

> TL;DR: [팀](reference.md)은 여러 단일 인스턴스를 **하나의 공유 채널**로 묶는
...
이 문서는 [Hermes 팀](reference.md#팀이-맥락을-공유하는-방법)의 "솔직한 현황"
...
[Hermes 팀](reference.md) 참고)가 하나의 Discord 채널을 공유합니다. 이들은
...
이것은 [Hermes 팀](reference.md)의 "먼저 키우고, 그 다음 묶어라" 규칙을 명시적인
핸드오프 프로토콜로 확장한 것입니다.
...
유일한 플랫폼**이고, [Hermes 팀](reference.md#라이브-증거)에 실린 리더 팀
...
| 에이전트 | Provider | 인증 | 비고 |
| --- | --- | --- | --- |
| `planner` | `litellm` | 프록시 키(`OPENAI_API_KEY`, 봉인) | 공유 LiteLLM 프록시에 OpenAI 호환 커스텀 provider로 연결; [`values-litellm.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/charts/hermes-agent/values-litellm.yaml) |
| `builder` | `copilot` | **device-flow**(봉인 토큰 없음) | 시작 시 `auth.deviceFlow`로 `gho_` 토큰 발급; [`values-github-copilot.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/charts/hermes-agent/values-github-copilot.yaml) |
...
- `releaseName` / `metadata.name`: 고유해야 함
  ([유일 규칙](https://github.com/jyje/hermes-agent-helm/blob/main/examples/argocd/README.md#the-one-rule-unique-fullname-per-instance))
...
[`values-multi-agent-collab.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/charts/hermes-agent/values-multi-agent-collab.yaml)을
...
남기면 - 팀원 추가가 한 줄 diff가 됩니다. 이는 [Hermes 팀](reference.md)의 팀
...
[`examples/argocd/hermes-collab-pair.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/examples/argocd/hermes-collab-pair.yaml)에
있습니다.
...
- [Hermes 팀](reference.md): 단일 인스턴스를 팀으로 묶기(이 문서의 전제).
- [로드맵](../../about/roadmap.md): ApplicationSet 팀 패턴과 `hermes-operator` 후보 조건.
