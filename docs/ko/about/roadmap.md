...
[English](../../about/roadmap.md) · [한국어](roadmap.md)
...
| **에이전트 팀 (ArgoCD ApplicationSet)** | 하나의 ApplicationSet으로 팀의 에이전트별 릴리즈를 생성(명부는 데이터, 이 차트는 템플릿): 공유 gateway 채널 1개, 멤버별 유일한 `fullname`, 팀원마다 Application 파일을 손으로 관리할 필요 없음 | 현재 | ✅ 권장: [Hermes 팀](../advanced/teams/reference.md) |
...
| 리더 주도 팀(Discord 스레드, 명시적 봇 멘션) | ✅ kind의 `v2026.7.20`에서 두 번 실증: 사람→리더→may→리더→march→리더→사람 완주, 마지막 턴에 멤버 멘션 없음, 두 번째 실행에서 마지막 줄 TEAM 메타데이터 확인; 전용 지식 마운트는 이후 추가되어 구조적으로 검증; [Hermes 팀](../advanced/teams/reference.md) 참고 |
...
- [Hermes 팀](../advanced/teams/reference.md): ApplicationSet 기반 팀 패턴 상세.
