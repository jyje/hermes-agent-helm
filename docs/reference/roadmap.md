...
| **Agent team (ArgoCD ApplicationSet)** | Generate a team's per-agent releases from one ApplicationSet (roster as data, the chart as the template) — one shared gateway channel, unique `fullname` per member, no hand-maintained Application files per teammate | now | ✅ recommended — [docs/reference/teams.md](teams.md) |
