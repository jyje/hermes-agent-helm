---
title: Hermes collaboration
description: Pair collaboration and handoff protocol.
---

# Hermes collaboration: make the bots talk to each other

[English](collaboration.md) · [한국어](../../../ko/advanced/teams/collaboration.md)

> TL;DR: A [team](reference.md) groups several single instances into **one shared
...
[Hermes teams](reference.md#how-a-team-shares-context): direct agent-to-agent awareness
...
identity - see [Hermes teams](reference.md)) share one Discord channel. They differ in
...
This is the same "scale up, then group" rule from [Hermes teams](reference.md), extended
...
[Hermes teams](reference.md#live-evidence)). The same `@mention` handoff and the same
...
| --- | --- | --- | --- |
| `planner` | `litellm` | proxy key (`OPENAI_API_KEY`, sealed) | talks to a shared LiteLLM proxy as an OpenAI-compatible custom provider; see [`values-litellm.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/charts/hermes-agent/values-litellm.yaml) |
| `builder` | `copilot` | **device-flow** (no token sealed) | mints a `gho_` token at startup via `auth.deviceFlow`; see [`values-github-copilot.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/charts/hermes-agent/values-github-copilot.yaml) |
...
- `releaseName` / `metadata.name`: must be unique (the
  [one rule](https://github.com/jyje/hermes-agent-helm/blob/main/examples/argocd/README.md#the-one-rule-unique-fullname-per-instance))
...
Copy [`values-multi-agent-collab.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/charts/hermes-agent/values-multi-agent-collab.yaml)
...
extends the team ApplicationSet in [Hermes teams](reference.md#3-or-generate-the-team-with-an-argocd-applicationset-recommended)
...
[`examples/argocd/hermes-collab-pair.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/examples/argocd/hermes-collab-pair.yaml)
...
- [Setting up a team](index.md): a friendlier, from-scratch
  walkthrough if this is your first team; start there.
- [teams.md](reference.md): group single instances into a team (the prerequisite to this page).
- [roadmap.md](../../about/roadmap.md): the ApplicationSet team pattern and the `hermes-operator` candidacy.
- [`values-multi-agent-collab.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/charts/hermes-agent/values-multi-agent-collab.yaml) · [`examples/argocd/hermes-collab-pair.yaml`](https://github.com/jyje/hermes-agent-helm/blob/main/examples/argocd/hermes-collab-pair.yaml)
