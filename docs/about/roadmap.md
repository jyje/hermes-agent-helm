...
| **Agent team (ArgoCD ApplicationSet)** | Generate a team's per-agent releases from one ApplicationSet (roster as data, the chart as the template): one shared gateway channel, unique `fullname` per member, no hand-maintained Application files per teammate | now | ✅ recommended: [Hermes teams](../advanced/teams/reference.md) |
...
- [Hermes teams](../advanced/teams/reference.md): the ApplicationSet-based team pattern in detail.
