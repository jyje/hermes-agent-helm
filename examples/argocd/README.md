...
These Application manifests deploy `hermes-agent` via ArgoCD safely — including
**multiple instances in the same namespace without collisions**.
...
`hermes-agent-staging-config`, `data-hermes-agent-staging-0`, …) — no overlap
with the `hermes-agent-*` set. Each gets its own PVC, so the two instances do
**not** share a knowledge base.
